/*
* USB device driver for Hilscher netX4000 based platforms
*
* drivers/usb/gadget/udc-netx4000.c
*
...
#define EP_INFO(_name, _maxpacket_limit, _buf_type) \
	{ \
		.name = _name, \
	...
	EP_INFO("ep1-bulk", 512, 1),
	EP_INFO("ep2-bulk", 512, 1),
	EP_INFO("ep3-bulk", 512, 0),
	EP_INFO("ep4-bulk", 512, 0),
	EP_INFO("ep5-bulk", 512, 0),

	EP_INFO("ep6-int", 1024, 0),
	EP_INFO("ep7-int", 1024, 0),
	EP_INFO("ep8-int", 1024, 0),
	EP_INFO("ep9-int", 1024, 0),

	EP_INFO("ep10-iso", 1024, 1),
	EP_INFO("ep11-iso", 1024, 1),
	EP_INFO("ep12-iso", 1024, 1),
	EP_INFO("ep13-iso", 1024, 1),

	/* The last two EP are available in HW but we have no buffer memory */
//	EP_INFO("ep14-iso", 1024, 1),
//	EP_INFO("ep15-iso", 1024, 1),
#undef EP_INFO
};
...
static int done(struct priv_usb_ep *_pep)
{
	...
	preq->req.complete(&_pep->ep, &preq->req);  /* giveback */
...
static int netx4000_udc_u2f_isr(int irq, void *_pdata);
static int netx4000_udc_u2f_epc_isr(int irq, void *_pdata);

static void netx4000_udc_poll(struct usb_gadget *_gadget)
{
	struct priv_data *pdata = gadget_to_pdata(_gadget);

	pr_debug("%s: %s()\n", DRIVER_NAME, __func__);

	netx4000_udc_u2f_isr(0, (void*)pdata);
	netx4000_udc_u2f_epc_isr(0, (void*)pdata);
}
...
static struct usb_request *netx4000_udc_alloc_request(struct usb_ep *_ep)
{
	struct priv_usb_request *preq;

	preq = xzalloc(sizeof(*preq));
	if (!preq) {
		pr_err("%s: failed at line=%d\n", __func__, __LINE__);
		return NULL;
	}
...
static int netx4000_udc_queue(struct usb_ep *_ep, struct usb_request *_req)
{
...
static int netx4000_udc_dequeue(struct usb_ep *_ep, struct usb_request *_req)
{
	...
	preq->req.complete(&pep->ep, &preq->req); /* giveback */
...
static int netx4000_udc_u2f_isr(int irq, void *_pdata)
{
	...
	if ((ioread32(EPCTR(pdata->ba)) & PLL_LOCK) == 0)
		return 0;
...
static int get_status(struct priv_data *_pdata, struct usb_ctrlrequest *_ctrl)
{
	...
	netx4000_udc_queue(pdata->gadget.ep0, &pdata->ep0_req.req);
...
static int netx4000_udc_probe(struct device_d *dev)
{
	struct priv_data *pdata;
	...
	if (IS_ERR(res_mem)) {
		pr_err("%s: dev_request_mem_resource() failed\n", __func__);
	...
	rc = usb_add_gadget_udc(dev, &pdata->gadget);
	if (rc) {
...
static struct driver_d netx4000_udc_driver = {
	.name = DRIVER_NAME,
	.probe = netx4000_udc_probe,
	.of_compatible = DRV_OF_COMPAT(netx4000_udc_match),
};
...
device_initcall(netx4000_udc_init);
